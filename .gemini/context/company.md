...
# Read: >
# Read: .company/artifacts/discovery/context.md
# Read: .company/artifacts/discovery/context.md)
# Read: .company/artifacts/cto/tech-stack.md)
# Read: .company/artifacts/architect/handoff-planning.md)
# Read: .company/artifacts/ui-designer/handoff-ui-design.md
# Read: .company/state.json)
...
**[Spawn Role: company-hiring-manager]**


Invoke the `/company-hiring-manager` command with:
> Assess expertise needs for project: {{args}}. Analyze requirements and identify which specialists we need.
...
BRANCH_NAME="feature/$(echo '{{args}}' | tr '[:upper:]' '[:lower:]' | tr ' ' '-' | sed 's/[^a-z0-9-]//g' | cut -c1-40)"
...
**[Spawn Role: company-cto]**


Invoke the `/company-cto` command with:
> Execute the task
...
**[Spawn Role: company-architect]**


Invoke the `/company-architect` command with:
> Execute the task
...
**[Spawn Role: company-ui-designer]**


Invoke the `/company-ui-designer` command with:
> Execute the task
...
**[Spawn Role: company-tech-lead]**


Invoke the `/company-tech-lead` command with:
> Execute the task
...
```
subject: "Feature: [name]", description: "...", owner: "developer"
```
```
...
**[Spawn Role: company-code-reviewer]**


Invoke the `/company-code-reviewer` command with:
> Review all implementation changes for: {{args}}
...
**[Spawn Role: company-qa]**


Invoke the `/company-qa` command with:
> Verify all implementations against acceptance criteria for: {{args}}
...
questions: [{
    header: "Merge Ready",
    question: "All quality gates passed for '{{args}}'. Approve merge to develop?",
    options: [
...
---
*Transpiled from: skills/company/SKILL.md*