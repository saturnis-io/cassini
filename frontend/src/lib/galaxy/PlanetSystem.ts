...
export class PlanetSystem {
  ...
  private buildFull(): void {
    ...
    const core = this.createCore(10.2)
  ...
  private buildDot(): void {
    ...
    this.dotSprite.scale.setScalar(1.5)
  ...
  private createCore(radius: number) {
  ...
  private createHaloPlanet() {
    ...
    const pPositions = new Float32Array(HALO_PLANET_PARTICLES * 3)
    const pColors = new Float32Array(HALO_PLANET_PARTICLES * 3)
    const phi = Math.PI * (3 - Math.sqrt(5))

    for (let i = 0; i < HALO_PLANET_PARTICLES; i++) {
      const y = 1 - (i / (HALO_PLANET_PARTICLES - 1)) * 2
      const r = Math.sqrt(1 - y * y)
      const theta = phi * i
  ...
  private createHaloCore() {
    const { planetRadius, colors } = this.config
    const radius = planetRadius * HALO_CORE_RADIUS_RATIO
  ...
  private createHaloRing() {
    ...
    const rPositions = new Float32Array(HALO_RING_PARTICLES * 3)
    const rColors = new Float32Array(HALO_RING_PARTICLES * 3)

    let ringIdx = 0
    while (ringIdx < HALO_RING_PARTICLES) {
      const theta = Math.random() * Math.PI * 2
      const radius = 12.0 + Math.pow(Math.random(), 1.2) * 20.0

      // Respect gaps even in halo mode for visual consistency
      let inGap = false
      for (const g of gaps) {
        if (radius > g.in && radius < g.out) {
          if (Math.random() > 0.015) inGap = true
    ...
    const mat = new THREE.ShaderMaterial({
      vertexShader: ringHaloVertexShader,
      fragmentShader: ringHaloFragmentShader,
      uniforms: {
        uPixelRatio: { value: Math.min(window.devicePixelRatio, 2) },
        uTime: { value: 0 },
      },
  ...
  update(time: number): void {
    ...
    // Halo level: update shader time + rotate planet, no moon logic
    if (this.currentLOD === 'halo') {
      if (this.haloRingMat) {
        this.haloRingMat.uniforms.uTime.value = time
      }
      if (this.haloPlanet) {
