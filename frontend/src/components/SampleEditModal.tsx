...
export function SampleEditModal({ isOpen, sample, onClose }: SampleEditModalProps) {
  ...
  const isValid =
    measurements.every((m) => m !== '' && !isNaN(parseFloat(m))) && reason.trim().length > 0
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* Modal */}
      <div className="bg-card border-border relative w-full max-w-lg rounded-xl border p-6 shadow-lg">
        ...
        <div className="space-y-4">
          ...
          {measurements.every((m) => m !== '' && !isNaN(parseFloat(m))) &&
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <span className="font-medium">
                      {(
                        measurements.reduce((a, b) => a + parseFloat(b), 0) / measurements.length
                      ).toFixed(4)}
                    </span>
                  </div>
                    ...
                    <div>
                      ...
                      <span className="font-medium">
                        {(
                          ...
                          Math.min(...measurements.map((m) => parseFloat(m)))
                        ).toFixed(4)}
                      </span>
                    </div>
                  )}
