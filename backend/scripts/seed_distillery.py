...
QC Lab and Barrel Aging. Designed to exercise non-normal distribution, short-run SPC, MSA, connectivity, and multivariate features:

 - Non-normal distributions (lognormal ABV, Weibull aging, Gamma color),
 custom Nelson rule presets, Laney p'/u' charts
 - Short-run charts (seasonal small-batch), Gage R&R data,
 FAI-style batch release
 - Gage connectivity (refractometer, hydrometer, GC, pH meter)
 - ERP-style batch IDs, LIMS lab certificates, mobile entry
 - Correlated multivariate (temp/humidity/ABV loss), predictive drift,
 ...
 *,
 ...
 # Non-normal - specific gravity is lognormal
 await create_variable_char(
 ...
 # Non-normal - ABV off still is Weibull-distributed
 await create_variable_char(
 ...
 # CUSUM for slow drift in congener levels
 await create_variable_char(
 ...
 # EWMA for column purity monitoring
 await create_variable_char(
 ...
 # Correlated - temp/humidity/ABV loss track together
 await create_variable_char(
 ...
 # Non-normal - angel's share (ABV loss) is gamma-distributed
 await create_variable_char(
 ...
 # Non-normal - barrel color development is lognormal
 await create_variable_char(
 ...
 # Short-run - limited edition single barrel
 await create_variable_char(
 ...
 # Laney p-chart - bottle visual inspection (overdispersed)
 p1_visual_data = []
 for i in range(100):
 ...
 # CUSUM for vodka methanol (ppm) - critical safety, slow drift detection
 await create_variable_char(
 ...
 # Short-run - limited-edition seasonal gin
 await create_variable_char(
 ...
 # Laney u-chart - taste panel defects (overdispersed)
 p2_taste_data = []
 for i in range(80):
 ...
 # Non-normal - ester production follows gamma distribution
 await create_variable_char(
 ...
 # Non-normal - tropical angel's share is more extreme
 await create_variable_char(
 ...
 # Short-run - aged reserve (very limited batches)
 await create_variable_char(
 ...
 # EWMA for slow fusel oil accumulation
 await create_variable_char(
 ...
 # Anejo aging (1-3 years) - Short-run for ultra-premium
 await create_variable_char(
 ...
 # Non-normal - smokiness is lognormal
 await create_variable_char(
 ...
 # Laney p-chart - agave piña quality (overdispersed)
 p4_agave_data = []
 for i in range(70):
 ...
 # Non-normal - furfural follows Weibull (from barrel aging)
 await create_variable_char(
 ...
 # CUSUM for methanol trending (lab confirmation of plant data)
 await create_variable_char(
 ...
 # Laney p-chart - sensory pass/fail (underdispersed, tight panel)
 p5_sensory_data = []
 for i in range(100):
 ...
 # Density/proof - Gage R&R style measurements (ready)
 await create_variable_char(
 ...
 # Short-run - special release proof verification
 await create_variable_char(
 ...
 *[{"rule_id": rid, "is_enabled": True, "parameters": None} for rid in range(3, 9)],
 ...
 print(" Non-normal (box_cox, weibull, gamma, percentile, auto)")
 print(" Laney p'/u' (overdispersed + underdispersed)")
 print(" Custom rule presets (4 presets)")
 print(" Short-run (deviation + standardized, 4 chars)")
 print(" Gage R&R ready (3-operator proof data)")
 print(" Gage bridge (QCL, 2 ports: GC-MS + HPLC)")
 print(" ERP webhook connector (HSD, 2 field mappings)")
 print(" CUSUM (3 chars), EWMA (3 chars)")
