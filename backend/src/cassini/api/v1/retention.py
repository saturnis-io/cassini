"""Retention policy REST endpoints for Cassini.
...
from cassini.api.schemas.retention import (
...
) -> PurgeHistoryResponse | dict:
    """Manually trigger a purge for a plant. Admin only.

    If a signature workflow is configured for ``retention_purge``, returns
    HTTP 202 with a ``workflow_instance_id`` — the purge is gated until
    the workflow completes.
    """
    check_plant_role(user, plant_id, "admin")

    # Gate purge behind signature workflow if configured
    sig_engine = SignatureWorkflowEngine(session)
    if await sig_engine.check_workflow_required(session, "retention_purge", plant_id):
        instance = await sig_engine.initiate_workflow(
            "retention_purge", plant_id, user.id, plant_id,
        )
        await session.commit()
        return {
            "status": "signature_required",
            "workflow_instance_id": instance.id,
            "message": "Electronic signature required before purge can execute",
        }
