...
from openspc.db.models.notification import SmtpConfig
...
class ReportScheduler:
    ...
    async def run_schedule(self, schedule_id: int) -> dict:
        ...
        try:
            # Generate the report
            from openspc.core.report_generator import generate_report
        ...
        try:
            ...
            for recipient in recipients:
                try:
                    ...
                    msg["From"] = smtp_from
                    msg["To"] = recipient
                    msg["Subject"] = f"[OpenSPC] Scheduled Report: {report_name}"

                    body = (
                        f"Please find attached the scheduled report: {report_name}\n\n"
                        f"Generated: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}\n"
                        f"This is an automated report from OpenSPC.\n"
                    )
