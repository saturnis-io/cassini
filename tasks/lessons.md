...
## L-007: Never Compute Capability Indices Client-Side — Use Backend Single Source of Truth (2026-03-06)
...
**Why it matters**: In a regulated SPC system, every displayed statistical value must be traceable to a single computation path. Client-side duplication of backend calculations introduces drift, especially when the data pipeline has multiple modes (subgroup means vs individual measurements, with/without chart options).

**Rule**: Statistical indices (Cpk, Ppk, Cp, Pp, Cpm) must ALWAYS come from the backend capability endpoint:
- Dashboard uses `useCapability(charId)` hook → `capability.cpk`, `capability.ppk`, etc.
- `<Explainable>` wrappers for capability metrics must NOT pass `chartOptions` — this ensures the explain API takes the same path as the capability endpoint (individual measurements + stored_sigma)
- The explain API "with chart options" path is ONLY for values computed from the chart view (subgroup means), not for capability indices

**Pattern to watch for**: "I'll compute it in the frontend to avoid an API call" — this is always wrong for statistical values. The backend is the single source of truth.

---

## L-008: External Y-Axis Domain Can Override Chart-Local Domain Logic (2026-03-06)

**Mistake**: Fixed spec limit toggle in ControlChart's own Z-scale domain branch, but the chart still didn't respond because `ChartPanel` passes an `externalDomain` computed by `calculateSharedYAxisDomain()` in `chart-domain.ts`. That function had the same bug — its Z-scale branch didn't include spec limits. The external domain always took precedence.

**Why it matters**: When debugging rendering issues, tracing only the component's internal logic misses the parent's domain override. The fix must be applied at all domain calculation sites.
